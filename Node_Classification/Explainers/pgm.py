## Source: PGM Explainer https://github.com/vunhatminh/PGMExplainer
...
class Node_Explainer:
    ...
    def extract_n_hops_neighbors(self, nA, node_idx):
        ...
        print(node_nA_row)
        neighbors = torch.nonzero(node_nA_row)
    ...
    def explain(self, node_idx, num_samples = 100, top_node = None, p_threshold = 0.05, pred_threshold = 0.1):
        ...
        x_sub, edge_index_sub, mapping, edge_mask, kwargs = self.__subgraph__(node_idx,x=self.X, edge_index=self.edge_index)
        A_nhop = to_dense_adj(edge_index_sub)
        A_nhop = torch.reshape(A_nhop, (A_nhop.shape[1], A_nhop.shape[2]))

        neighbors = torch.nonzero(nA[node_idx])

        #
        if (node_idx not in neighbors):
            neighbors = torch.cat((neighbors, [torch.tensor(node_idx)]))
        ...
        pred_torch = self.model.forward(X_torch, edge_index)
        #pred_torch = pred_torch.cpu()
        soft_pred = pred_torch
        #soft_pred = F.softmax(pred_torch,dim=2)
        pred_node = pred_torch[node_idx].data
        label_node = torch.argmax(pred_node)
        soft_pred_node = soft_pred[node_idx]
        Samples = []
        Pred_Samples = []

        for iteration in range(num_samples):

            X_perturb = self.X.clone()
            sample = []
            ...
            X_perturb_torch = X_perturb_torch.cuda()
            A_torch = A_torch.cuda()
            pred_perturb_torch = self.model.forward(X_perturb_torch, edge_index)
            soft_pred_perturb = pred_perturb_torch
            sample_bool = []
            for node in neighbors:
                if (soft_pred_perturb[node, torch.argmax(soft_pred[node])] + pred_threshold) < torch.max(soft_pred[node]).item():
        ...
        for s in range(Samples.shape[0]):
            ...
            #Combine_Samples[s] = Samples[s]*10 + Pred_Samples[s]+1
        #Combine_Samples = Combine_Samples.numpy()
        data = pd.DataFrame(Combine_Samples)

        ind_sub_to_ori = dict(zip(list(data.columns), neighbors.cpu().numpy().reshape(neighbors.shape[0])))
        data = data.rename(columns={0: "A", 1: "B"}) # Trick to use chi_square test on first two data columns

        ind_ori_to_sub = dict(zip(neighbors.cpu().numpy().reshape(neighbors.shape[0]),list(data.columns)))
        p_values = []
        dependent_neighbors = []
        dependent_neighbors_p_values = []
        neighbors=neighbors.cpu().numpy().reshape(neighbors.shape[0])
        for node in neighbors:
        ...
        #print('dependent_neigh',dependent_neighbors)
        ...
        #
        num_computation_graph_nodes = A_nhop.shape[0]
        nodes_mask = np.zeros(num_computation_graph_nodes)
        for i in range(num_computation_graph_nodes):
            if ind_sub_to_ori[i] in pgm_nodes:
                nodes_mask[i]=1
        


        return pgm_nodes, nodes_mask
